...
module AgilewayUtils
  ...
  def use_current_browser
    ...
    elsif ENV['BROWSER'] == 'firefox' || ENV['BROWSER'] == 'safari' || ENV['BROWSER'] == 'ie'
    ...
    # @driver = Selenium::WebDriver.for(:remote, :url => "http://localhost:7055/hub", :desired_capabilities => caps)
      ...
      port_set_in_env_var = begin
        ENV['BROWSER_DEBUGGING_PORT'].to_i
      rescue StandardError
        0
      end
      if browser_debugging_port && browser_debugging_port > 1000 && browser_debugging_port < 65_500
        # OK to used from pstore
        puts('[DEBUG] use debugging port from TestWise pstore')
      elsif port_set_in_env_var > 1000 && port_set_in_env_var < 65_500
        ...
        browser_debugging_port = 19_218
      end
      puts(" => #{browser_debugging_port}")

      the_edge_options.debugger_address = "127.0.0.1:#{browser_debugging_port}"

      the_browser_options = if defined?(TestwiseListener)
                              { options: the_edge_options, listener: TestwiseListener.new }
                            else
      ...
      the_browser_options = {}
      the_browser_options = if defined?(TestwiseListener)
                              { options: the_chrome_options, listener: TestwiseListener.new }
                            else
  ...
  def try_for(timeout = $testwise_polling_timeout, polling_interval = $testwise_polling_interval || 1)
    start_time = Time.now
  ...
  def take_screenshot(opts = {})
    ...
    rescue ::DL::DLTypeError => e
      puts "No screenshot libray found: #{e}"
    rescue StandardError => e
  ...
  def days_before(days, format = nil)
    return nil unless days.instance_of?(Integer)
  ...
  def random_char(lowercase = true)
    if lowercase
      format('%c', random_number(97, 122))
    else
      format('%c', random_number(65, 90))
    end
  end

  def random_digit
    format('%c', random_number(48, 57))
  end
  ...
  #   }
  #
  def process_each_row_in_csv_file(csv_file)
    require 'faster_csv'
